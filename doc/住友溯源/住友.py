...
site_file_path = r'data_sumitomo\sites.xlsx'
...
site_coordinates_cleaned['X'] = (site_coordinates_cleaned['Longitude'] - boundary_coords['top_left'][0]) / lon_range * 1000
site_coordinates_cleaned['Y'] = (site_coordinates_cleaned['Latitude'] - boundary_coords['bottom_left'][1]) / lat_range * 1000

# Step 3: Load pollution monitoring data for the three stations
monitoring_data_file_path = r"data_sumitomo\厂界\test3\hourly_averaged_data.xlsx"
monitoring_data = pd.read_excel(monitoring_data_file_path)
monitoring_data.columns = ['时间', 'N', 'S', 'E']

# Step 4: Load wind data
wind_file_path = r"data_sumitomo\厂界\test3\wind.xlsx"
...
def gaussian_plume(x, y, Q, u, sigma_y, sigma_z, H=0):
    C = (Q / (2 * np.pi * u * sigma_y * sigma_z)) * np.exp(-y**2 / (2 * sigma_y**2)) * np.exp(-H**2 / (2 * sigma_z**2))
    return C

def calculate_concentration(x_source, y_source, u, wind_dir, station_x, station_y):
    wind_dir_rad = np.deg2rad(wind_dir)
    x_downwind = (station_x - x_source) * np.cos(wind_dir_rad) + (station_y - y_source) * np.sin(wind_dir_rad)
    y_crosswind = -(station_x - x_source) * np.sin(wind_dir_rad) + (station_y - y_source) * np.cos(wind_dir_rad)
...
def fitness_function(source_pos, wind_data, site_coordinates_cleaned, observed_concentrations, Q=1.0):
    total_error = 0
    for i in range(len(wind_data)):
        for idx, row in site_coordinates_cleaned.iterrows():
            predicted_concentration = calculate_concentration(source_pos[0], source_pos[1],
                                                              wind_data.iloc[i]['sp'],
...
def ssa_optimize(max_iter, population, fitness_function):
    for iteration in range(max_iter):
        fitness_scores = [fitness_function(pos, wind_data, site_coordinates_cleaned, monitoring_data) for pos in population]
...
site_coordinates_cleaned['Latitude_plot'], site_coordinates_cleaned['Longitude_plot'] = zip(*site_coordinates_cleaned.apply(
    lambda row: xy_to_latlon(row['X'], row['Y'], boundary_coords['top_left'], boundary_coords['top_right'], boundary_coords['bottom_left']), axis=1))

best_source_lat, best_source_lon = xy_to_latlon(best_source_position[0], best_source_position[1],
                                                boundary_coords['top_left'], boundary_coords['top_right'], boundary_coords['bottom_left'])

# 打印推论的污染源经纬度坐标
print(f"Estimated Pollution Source Location: Latitude = {best_source_lat}, Longitude = {best_source_lon}")

plt.figure(figsize=(8, 8))
plt.plot([boundary_coords['top_left'][0], boundary_coords['top_right'][0], boundary_coords['top_right'][0], boundary_coords['top_left'][0], boundary_coords['top_left'][0]],
         [boundary_coords['top_left'][1], boundary_coords['top_left'][1], boundary_coords['bottom_left'][1], boundary_coords['bottom_left'][1], boundary_coords['top_left'][1]], 
         'k-', label='Research Area')

for idx, row in site_coordinates_cleaned.iterrows():
    plt.scatter(row['Longitude_plot'], row['Latitude_plot'], c='blue', label=f'Station {row["Station"]}' if idx == 0 else "", s=100)

plt.scatter(best_source_lon, best_source_lat, c='red', label='Estimated Pollution Source', s=200)

# 绘制半径50米的区域
circle = Circle((best_source_lon, best_source_lat), radius=50 / 111000, color='red', fill=False, linestyle='--', label='Source Area (50m radius)')
