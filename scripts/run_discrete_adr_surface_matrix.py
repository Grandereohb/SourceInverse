"""Run frozen conventional discrete-ADR candidate surfaces over a matrix."""
...
RUNNER = ROOT / "scripts" / "run_discrete_adr_baseline.py"
CODE_FILES = (
    "pinn_source/data_io.py",
    "pinn_source/discrete_adr_baseline.py",
    "pinn_source/gaussian_puff_baseline.py",
    "scripts/run_discrete_adr_baseline.py",
    "scripts/run_discrete_adr_surface_matrix.py",
)
...
def main() -> None:
    ...
    plan_comparable = {
        "schema_version": 1,
        "experiment_type": "truth_blind_conventional_discrete_ADR_surface_matrix",
        "batch_manifest": {
            "path": str(args.batch_manifest.resolve()),
            "sha256": _sha256(args.batch_manifest),
        },
        "code_snapshot": {
            relative: _sha256(ROOT / relative) for relative in CODE_FILES
        },
        "settings": {
            ...
            "transport_pad_m": args.transport_pad_m,
        ...
        "truth_isolation": "runner resolves scenario inputs but never reads source coordinates",
    }
    plan_path = args.output_root / "experiment_plan.json"
    if plan_path.exists():
        existing = json.loads(plan_path.read_text(encoding="utf-8"))
        existing.pop("created_at_utc", None)
        if existing != plan_comparable:
            raise ValueError("existing discrete ADR plan does not match invocation")
    else:
        plan = dict(plan_comparable)
    ...
    for entry in batch["scenarios"]:
        ...
        output = args.output_root / scenario_id / "discrete_adr_candidate_surfaces.json"
        if not output.is_file():
            ...
            command = [
                ...
                "--output", str(output),
                "--diffusivity-m2s", str(args.diffusivity_m2s),
                "--initial-sigma-m", str(args.initial_sigma_m),
                "--decay-per-hour", str(args.decay_per_hour),
                "--wind-factor", str(args.wind_factor),
                "--source-position-pad-m", str(args.source_position_pad_m),
                "--transport-pad-m", str(args.transport_pad_m),
                ...
                "--transport-grid-nx", str(args.transport_grid_nx),
                "--transport-grid-ny", str(args.transport_grid_ny),
                "--save-candidates",
            ]
